module.exports.config = {
    ...
    Rent: 2,
    credits: "Vtuan",
    description: "Spam a content to death (continuous summoning)",
  ...
  module.exports.run = async function({ api, event, args }) {
    ...
    if (args[0] === "stop") {
      ...
      if (state) {
        ...
        api.sendMessage("No one has been tagged!", threadID, messageID);
      }
      return;
    }
    const content = (args.length != 0) ? args.join(" ") : ``
    var mention = Object.keys(event.mentions)[0];
    if (!mention) return api.sendMessage("Need to tag 1 person!", event.threadID);
