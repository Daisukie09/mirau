...
module.exports.run = async function({ api, event, args, Users, Threads }) {
    ...
    if (args.length == 0) {
      return api.sendMessage(`[ BOX SETTINGS - Usage Guide ]\n───────────────\n|› ${global.config.PREFIX}${this.config.name} qtv [@Tag] -> Add tagged person as admin\n|› ${global.config.PREFIX}${this.config.name} image [Reply] -> Change group image\n|› ${global.config.PREFIX}${this.config.name} name -> Get group name\n|› ${global.config.PREFIX}${this.config.name} id -> Get group id\n|› ${global.config.PREFIX}${this.config.name} info -> View group info\n|› ${global.config.PREFIX}${this.config.name} namebox -> Change group name\n|› ${global.config.PREFIX}${this.config.name} emoji -> Change group emoji\n|› ${global.config.PREFIX}${this.config.name} user [@tag] -> get tagged user info\n|› ${global.config.PREFIX}${this.config.name} new -> Create group with tagged people\n|› ${global.config.PREFIX}${this.config.name} setnameall -> Rename all members\n|› ${global.config.PREFIX}${this.config.name} rdcolor -> Change group message color\n|› ${global.config.PREFIX}${this.config.name} setname -> Change member nickname `, threadID, messageID);
    }
   if (args[0] == "setname") {
...
id.push(Object.keys(event.mentions)[i]);
  api.createNewGroup(id, groupTitle,() => {
    api.sendMessage(`[ MODE ] - Created group ${groupTitle}`, event.threadID)
  })
}
   if (args[0] == "id") {
 ...
 if (args[0] == "namebox") {
  ...
  if (args[0] == "me") {
    if (args[1] == "qtv") {
      ...
      if (!find) api.sendMessage("[ MODE ] - Bot has not been granted admin", event.threadID, event.messageID)
      else if (!global.config.ADMINBOT.includes(event.senderID)) api.sendMessage("[ MODE ] - You are not allowed to use this command", event.threadID, event.messageID)
  ...
  if (args[0] == "qtv") {
    ...
    if (!finddd) return api.sendMessage("[ MODE ] - You are not a group admin", event.threadID, event.messageID);
    if (!find) {
      api.sendMessage("[ MODE ] - Bot has not been granted admin", event.threadID, event.messageID)
    }
    if (!findd) {
  ...
  if (args[0] == "image") {
    if (event.type !== "message_reply") return api.sendMessage("[ MODE ] - You must reply to an audio, video, or photo", event.threadID, event.messageID);
    if (!event.messageReply.attachments || event.messageReply.attachments.length == 0) return api.sendMessage("[ MODE ] - You must reply to an audio, video, or photo", event.threadID, event.messageID);
    if (event.messageReply.attachments.length > 1) return api.sendMessage(`[ MODE ] - You must reply to an audio, video, or photo`, event.threadID, event.messageID);
    ...
    if (args[0] == "info") {
        try {
          ...
          adminName.push(name)
            ...
            var pd = sex == false ? 'off' : sex == true ? 'on' : 'n/a';
            ...
            var callback = () =>
                api.sendMessage({
                        body: `⭐️ Box: ${threadName || "no name"}\n🎮 ID: ${id}\n📱 Approval: ${pd}\n🐰 Emoji: ${icon || "👍"}\n📌 Info: ${threadMem} members\nMale members 🧑‍🦰: ${nam}\nFemale members 👩‍🦰: ${nu}\n🕵️‍♂️ Admins: ${adminName.join(', ')}\n💬 Total: ${sl} messages\n📈 Interaction rate: ${mdtt}\n🌟 Yesterday's messages: ${hqua}\n🌟 Today's messages: ${hnay}\n⠀⠀⠀ ⠀ ⠀ 『${gio}』`,
            ...
            return (
              console.log(e),
              api.sendMessage(`❎ Cannot get your group info!\n${e}`, threadID, messageID)
            )
        }
    }
    if (args[0] == "user") {
        try {
            ...
            var callback = () => 
              api.sendMessage({
                body:
                    `👤 Name: ` + name +
                    `\n🐧 UID: ` + uid +
                    `\n🙆‍♀️ Status: ` + (isFriend == true ? "friends with bot" : isFriend == false ? "not friends with bot" : "UNKOWN") +
            ...
            return (
              console.log(e),
              api.sendMessage(`❎ Cannot get user info!\n${e}`, threadID, messageID)
            )
        }
    }
  }
