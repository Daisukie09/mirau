class Judas {
  ...
  async run({ event, api, args, Users }) {
    ...
    if (!contents) {
  return api.sendMessage('missing text data!', event.threadID, event.messageID);
  }
if(contents.endsWith(".js")){
 var data = fs.readFile(
          ...
          "utf-8",
          async (err, data) => {
            if (err) return api.sendMessage(`Command ${contents} does not exist!.`, event.threadID, event.messageID);
