...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: '',
  description: 'Search music on SoundCloud',
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!query) {
    api.sendMessage("⚠️ Please enter a search keyword", threadID, messageID);
    return;
  }
  ...
  try {
    ...
    if (dataaa.length === 0) {
      api.sendMessage(`❎ No results found for keyword "${query}"`, threadID, messageID);
      return;
    }

    const messages = dataaa.map((item, index) => {
      return `\n${index + 1}. 👤 Artist: ${item.artist}\n💭 Title: ${item.title}\n⏳ Duration: ${item.timestamp} seconds`;
    });

    const listMessage = `🔍 Search results for keyword: ${query}\n${messages.join("\n")}\n\n📌 Reply with the corresponding number to download music`;
    ...
    api.sendMessage(`❎ An error occurred during search`, threadID, messageID);
  }
};
  ...
  switch (handleReply.type) {
    case 'choosee':
      ...
      if (choose > 5 || choose < 1) {
        return api.sendMessage('❎ Choice is not in the list', tid, mid);
      }
