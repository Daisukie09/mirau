...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      reply(`📌 Reply to this message with the ${["01", "1"].includes(args[0]) ? "bio" : "nickname"} you want to change for the bot or 'delete' if you want to delete the current ${["01", "1"].includes(args[0]) ? "bio" : "nickname"}`, (err, info) => {
      ...
      return reply(`📌 List of pending messages of the bot:\n\n${msg}`);
    }
    else if (["04", "4"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["unread"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | Message: ${b.snippet}\n`, "") || "❎ No messages";
      return reply(`📌 List of unread messages of the bot:\n\n${msg}`);
    }
    else if (["05", "5"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["OTHER"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | Message: ${b.snippet}\n`, "") || "❎ No messages";
      return reply(`List of spam messages of the bot:\n\n${msg}`);
    }
    else if (["06", "6"].includes(args[0])) {
      reply(`📌 Reply to this message with an image or image link to change the bot's avatar`, (err, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      if (!args[1] || !["on", "off"].includes(args[1])) return reply('❎ Please choose on or off');
      const form = {
        av: botID,
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (err, data) => {
        ...
        else reply(`✅ Successfully ${args[1] == 'on' ? 'enabled' : 'disabled'} the bot's avatar shield`);
      });
    }
    else if (["08", "8"].includes(args[0])) {
      return reply(`📌 Reply to this message with the id of the person you want to block, you can enter multiple ids separated by spaces or new lines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      return reply(`📌 Reply to this message with the id of the person you want to unblock, you can enter multiple ids separated by spaces or new lines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      return reply(`📌 Reply to this message with the post id you want to delete, you can enter multiple ids separated by spaces or new lines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      return reply(`📌 Reply to this message with the postID you want to comment on (${args[0] == "12" ? "user's post" : "group post"}), you can enter multiple ids separated by spaces or new lines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      reply(`📌 Reply to this message with the ${args[0] == "14" ? "post" : "user"} id you want to ${args[0]  == "13" ? "send friend request" : args[0] == "14" ? "react" : args[0] == "15" ? "send friend request" : args[0] == "16" ? "accept friend request" : args[0] == "17" ? "decline friend request" : args[0] == "18" ? "unfriend" : "send message"}, you can enter multiple ids separated by spaces or new lines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    api.changeBio(bio, false, (err) => {
      ...
      else return reply(`✅ Successfully ${!bio ? "deleted the bot's bio" : `changed the bot's bio to: ${bio}`}`);
    });
  }
    ...
    if (nickname) {
      ...
      if (!res.includes('href="/profile/edit/info/nicknames/?entid=')) return reply('❎ Your bot currently has no nickname set');
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      ...
      else reply(`✅ Successfully ${!nickname ? "deleted the bot's nickname" : `changed the bot's nickname to: ${nickname}`}`);
    });
  }
    ...
    else return reply(`❎ Please enter a valid image link or reply to this message with an image to set as bot's avatar`, (err, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    try {
      ...
      if (uploadImageToFb.error) return reply("❎ An error occurred: " + uploadImageToFb.error.errorDescription);
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
        ...
        else reply(`✅ Successfully changed the bot's avatar`);
      });
    }
    catch(err) {
    ...
    if (!body) return reply("📌 Please enter the uid of the people you want to block on messenger, you can enter multiple ids separated by spaces or new lines", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`✅ Successfully blocked ${success.length} users on messenger${failed.length > 0 ? `\n❎ Failed to block ${failed.length} users, id: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'unBlockUser') {
    if (!body) return reply("📌 Please enter the uid of the people you want to unblock on messenger, you can enter multiple ids separated by spaces or new lines", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`✅ Successfully unblocked ${success.length} users on messenger${failed.length > 0 ? `\n❎ Failed to unblock ${failed.length} users, id: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'createPost') {
    if (!body) return reply("📌 Please enter the content you want to create a post", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
      if (e || JSON.parse(i).errors) return reply(`❎ Failed to create post, please try again later`);
    ...
    reply(`✅ Successfully commented on ${success.length} posts${failed.length > 0 ? `\n❎ Failed to comment on ${failed.length} posts, postID: ${failed.join(" ")}` : ""}`);
  }
    ...
    for (const postID of postIDs) {
  		...
  		try {
           ...
           headers
   		  ...
   		  reply("❎ An error occurred, post id does not exist or you are not the owner of this post");
  		}
      ...
      const data = "fb_dtsg=" + encodeURIComponent(fb_dtsg) +"&jazoest=" + encodeURIComponent(jazoest) + "&action_key=DELETE&submit=G%E1%BB%ADi";
        ...
        const dt = await axios({
   			...
   			if (dt.data.includes("❎ Sorry, an error occurred")) throw new Error();
  			success.push(postID);
  		}
  		catch(err) {
  			failed.push(postID);
    ...
    reply(`✅ Successfully deleted ${success.length} posts${failed.length > 0 ? `\n❎ Failed to delete ${failed.length} posts, postID: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdReactionPost') {
    if (!body) return reply(`📌 Please enter the id of the post you want to react to`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`📌 Enter the reaction you want for ${listID.length} posts (unlike/like/love/heart/haha/wow/sad/angry)`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!'unlike/like/love/heart/haha/wow/sad/angry'.split('/').includes(feeling)) return reply('❎ Please choose one of the following reactions unlike/like/love/heart/haha/wow/sad/angry', (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`✅ Successfully reacted ${feeling} to ${success.length} posts${failed.length > 0 ? `\n❎ Failed to react to ${failed.length} posts, postID: ${failed.join(" ")}` : ''}`);
  }
    ...
    reply(`✅ Successfully sent friend requests to ${success.length} ids${failed.length > 0 ? `\n❎ Failed to send friend requests to ${failed.length} ids: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdSendMessage') {
    const listID = body.replace(/\s+/g, " ").split(" ");
    reply(`📌 Enter the message content you want to send to ${listID.length} users`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`✅ Successfully removed ${success.length} friends${failed.length > 0 ? `\n❎ Failed to remove ${failed.length} friends:\n${failed.join("\n")}` : ""}`);
  }
    ...
    reply(`✅ Successfully sent message to ${success.length} users${failed.length > 0 ? `\n❎ Failed to send message to ${failed.length} users: ${failed.join(" ")}` : ""}`);
  }
    ...
    reply(`✅ Successfully ${type == 'acceptFriendRequest' ? 'accepted' : 'deleted'} friend requests of ${success.length} ids${failed.length > 0 ? `\n❎ Failed for ${failed.length} ids: ${failed.join(" ")}` : ""}`);
  }
    ...
    .then(response => {
      ...
      reply(`Note created successfully, link: ${'https://buildtool.dev/code-viewer.php?' + href}`)
    })
    .catch(err => {
      reply('An error occurred, please try again later');
    })
...
module.exports.run = async ({ event, api }) => {
  ...
  api.sendMessage("[ Setting Account List ]\n\n"
     ...
     + "\n07. Toggle bot avatar shield on/off"
     ...
     + "\n18. Remove friend by id"
     + "\n19. Send message by id"
     + "\n20. Create note on buildtool.dev"
     + "\n21. Logout account"
    + `\n\n📌 Reply to this message with the number of the command you want to execute`, threadID, (err, info) => {
    global.client.handleReply.push({
      name: this.config.name,
      messageID: info.messageID,
      author: senderID,
