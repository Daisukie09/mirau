...
module.exports.run = async function ({ api, event, args }) {
  try {
    ...
    if (!keyword) {
      api.sendMessage("⚠️ Please enter a keyword to search for music on Spotify", event.threadID, event.messageID);
      return;
    }
    ...
    if (dataSearch.length === 0) {
      api.sendMessage(`❎ No results found for keyword: ${keyword}`, event.threadID, event.messageID);
      return;
    }
...
💽 Size: ${sizeFormatted}`;
          ...
          // Revoke message when processing is done
    ...
    api.sendMessage('⚠️ Please enter a valid number from the list', event.threadID, event.messageID);
  }
};