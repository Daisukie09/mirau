module.exports.config = {
    ...
    description: "Full feature shortcut",
    commandCategory: "Utility",
    usages: "[all/delete/empty/tag/join/leave/autosend]\n{time} -> get time\n{name} -> get name user\n{nameThread} -> get name box\n{soThanhVien} -> get group member count\n{link} -> get link user\n{authorName} -> get name of person who added or kicked\n{authorId} -> get link of person who added or kicked\n{trangThai} -> shows self-leave or was kicked by admin\n{qtv} -> when member joins or leaves, tag all admins",
    cooldowns: 0,
    dependencies: {
        "fs-extra": "",
        "path": ""
...
// Removed the require override code that caused errors
        ...
        case 'log:unsubscribe': {
            ...
            const msg = {
                body: leave.output
                    ...
                    .replace(/{time}/g, require('moment-timezone')().tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY - HH:mm:ss'))
                    .replace(/{trangThai}/g, logMessageData.leftParticipantFbId == author ? `has left the group` : `was kicked from the group`)
                    .replace(/{authorName}/g, global.data.userName.get(author))
                    .replace(/{authorId}/g, `https://www.facebook.com/profile.php?id=${author}`)
                    .replace(/{qtv}/g, `@${admins.map(e => e[1]).join('\n@')}`)
            };
...
module.exports.handleEvent = async function ({ event, api, Users }) {
    ...
    // if (api.getCurrentUserID()==senderID)return; // Removed so bot can reply to its own messages
    if (!global.moduleData.shortcut) global.moduleData.shortcut = new Map();
    ...
    if ((dataThread = mentions.length > 0 ? data.find(item => typeof item.tag_id == 'string' && mentions.includes(item.tag_id)) : false) || (dataThread = data.find(item => (item.input || '').toLowerCase() == body.toLowerCase()))) {
        ...
        // Assume global.khanhdayr already has a value (assigned from api_link() or elsewhere)
        // console.log(global.khanhdayr);  // Debug log disabled

        // Process message content (msg) and attachments
        if (dataThread.uri === 'random') {
            msg.attachment = [await stream_url(global.khanhdayr)];  // Call global.khanhdayr directly
        } else if (/^https:\/\//.test(dataThread.uri)) {
            msg.attachment = [await stream_url(dataThread.uri)];
        }

        // Check if msg has attachment
        // console.log(msg.attachment);  // Debug log disabled

        // Return message with content and attachment
        return api.sendMessage({
            body: msg.body,  // Message content (msg.body)
            attachment: global.khanhdayr.splice(0, 1)  // Attachment from global.khanhdayr
        }, threadID, messageID);
    }
}
module.exports.handleReply = async function ({ event = {}, api, handleReply }) {
    ...
    try {
        ...
        switch (handleReply.type) {
            case "requireInput": {
                if (body.length == 0) return api.sendMessage("❎ Answer cannot be empty", threadID, messageID);
                const data = global.moduleData.shortcut.get(threadID) || [];
                if (data.some(item => item.input == body)) return api.sendMessage("❎ Input already exists", threadID, messageID);
                api.unsendMessage(handleReply.messageID);
                return api.sendMessage("📌 Reply to this message to enter the answer when using the keyword", threadID, function (error, info) {
            ...
            case "requireOutput": {
                if (body.length == 0) return api.sendMessage("❎ Answer cannot be empty", threadID, messageID);
                api.unsendMessage(handleReply.messageID);
                return api.sendMessage(`📌 Reply to this message with a video/photo/mp3/gif file or if not needed you can reply to this message and enter 's' or if you want random video from available api data then enter 'random'`, threadID, function (error, info) {
                    return global.client.handleReply.push({
                        type: "requireGif",
                        name,
                        author: senderID,
            ...
            case "requireGif": {
                ...
                if (!['s', 'random'].includes(body)) {
                    ...
                    if (attachments.length === 0) return api.sendMessage('⚠️ no attachment file entered', threadID, messageID);
                    ...
                    try {
                        ...
                        console.error(e);
                        return api.sendMessage('⚠️ Unable to upload', threadID, messageID);
                    };
                };
            ...
            case "delShortcut": {
                ...
                break;
            case 'autosend': {
                if (!body) return api.sendMessage('⚠️ No content entered', threadID, messageID);

                api.sendMessage('📌 Please reply to this message with the time\nExample: 12:00:00', threadID, (err, data) => {
            ...
            case 'autosend.input_time': {
                if (!require('moment-timezone')(body, 'HH:mm:ss').isValid() || body.length !== '00:00:00'.length) return api.sendMessage('⚠️ Invalid time', threadID, messageID);

                api.sendMessage(`📌 Reply to this message with a video/photo/mp3/gif file or if not needed you can reply to this message and enter 's' or if you want random video from available api data then enter 'random'`, threadID, (err, data) => {
                    global.client.handleReply.push({
                        ...data,
                        author: senderID,
                        name: exports.config.name,
            ...
            case 'autosend.input_attachment': {
                ...
                api.sendMessage('✅ Auto send added', threadID, messageID);
            }
                break;
            case 'join':
            case 'leave': {
                if (!handleReply.data.output) {
                    ...
                    api.sendMessage('✅ Added short ' + handleReply.type, threadID, messageID);
                }
            }
                break;
            default:
...
module.exports.run = function ({ event, api, args }) {
    try {
        ...
        switch (args[0]) {
            case 'join':
            case 'leave': {
                api.sendMessage(`📌 Please reply to this message and enter the content`, threadID, (err, data) => {
                    global.client.handleReply.push({
                        ...data,
                        author: senderID,
                        name: exports.config.name,
                ...
                break;
            case 'autosend': {
                api.sendMessage(`📌 Please reply to this message and enter auto-send content (add | each content for random) \nExample: good morning | good morning sunshine`, threadID, (err, data) => {
                    global.client.handleReply.push({
                        ...data,
                        author: senderID,
                        name: exports.config.name,
            ...
            case "-d": {
                ...
                if (indexData == -1) return api.sendMessage("❎ Your group currently has no shortcuts set", threadID, messageID);
                var dataThread = data.find(item => item.threadID == threadID) || { threadID, shortcuts: [] };
                var dataGlobal = global.moduleData.shortcut.get(threadID) || [];
                //var indexNeedRemove;

                if (dataThread.shortcuts.length == 0) return api.sendMessage("❎ Your group currently has no shortcuts set", threadID, messageID);
                /*
            ...
            case "-a": {
                const data = global.moduleData.shortcut.get(threadID) || [];
                var array = [];
                if (data.length == 0) return api.sendMessage("❎ Your group currently has no shortcuts set", threadID, messageID);
                else {
                    var n = 1;
                    ...
                    return api.sendMessage(`📝 Below are all of the group's shortcuts: \n\n${array.join("\n")} \n\n'yes' means has attached file\n'no' means no attached file\n\nReply by number to delete shortcut`, threadID, function (error, info) {
                        global.client.handleReply.push({
                            type: "delShortcut",
                            name,
                            author: senderID,
            ...
            case 'tag': {
                ...
                const data = global.moduleData.shortcut.get(threadID) || [];
                if (data.some(item => item.tag_id == tag_id)) return api.sendMessage("❎ Tag already exists", threadID, messageID);

                api.sendMessage("📌 Reply to this message to enter the answer when tagged", threadID, function (error, info) {
                    global.client.handleReply.push({
                        type: "requireOutput",
                        name,
                        author: senderID,
                ...
                break;
            default: {
                return api.sendMessage("📌 Reply to this message to enter the keyword for the shortcut", threadID, function (error, info) {
                    return global.client.handleReply.push({
                        type: "requireInput",
                        name,
                        author: senderID,
...
/*async function api_link() {
  return (await require('axios').get('https://niiozic.site/girl-video')).data.url;
}*/
