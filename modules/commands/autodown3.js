        ...
        if (media.url) {
            if (media.type === 'video') {
                try {
                    ...
                    console.error("Error downloading video from Threads:", error);
                }
                ...
                try {
                    ...
                    console.error("Error downloading image from Threads:", error);
                }
            }
        }
    }
...
exports.config = {
    ...
    hasPermssion: 0,
    credits: 'DGK',
    description: 'Auto download video and images from Douyin, Weibo, xhslink, Twitter, Instagram, Threads.',
...
exports.handleEvent = async function (o) {
    ...
    // console.log("Input text:", text); // Spam log disabled

    // const regexXhslink = /http:\/\/xhslink\.com\/a\/[A-Za-z0-9]+/;
    // const matchXhslink = text.match(regexXhslink);
    // if (matchXhslink) {
    //     console.log("Found xhslink link:", matchXhslink[0]);
    // } else {
    //     console.log("No xhslink link found.");
    // }

    let urls = extractUrls(text);
    // console.log("Found URLs:", urls); // Spam log disabled
    ...
    if (!douyinUrl && !weiboUrl && !xhslinkUrl && !twitterUrl && !instagramUrl && !threadsUrl) {
        // console.log("No valid URLs found."); // Spam log disabled
        return;
    }

    let mediaUrl = douyinUrl || weiboUrl || xhslinkUrl || twitterUrl || instagramUrl || threadsUrl;
        // console.log("Processed URL:", mediaUrl); // Spam log disabled

    try {
        let res = await axios.get(`http://sv.gamehosting.vn:31217/media?url=${encodeURIComponent(mediaUrl)}`);
        // console.log("Data received from API:", res.data); // Spam log disabled

        if (!res.data || !res.data.medias) {
            console.error('API data parse error.');
            return;
        }
            ...
            if (douyinUrl) {
                ...
                if (videoMedia && videoMedia.url) {
                    try {
                        ...
                        console.error("Error downloading video from Instagram:", error);
                    }
                } else {
                    console.log("No video found in Instagram data.");
                }
            }
        }
...
`;
        ...
        console.error("Error calling API or processing data:", err);
    }
};