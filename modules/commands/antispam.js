...
module.exports.config = {
	...
	hasPermssion: 1,
	credits: "BraSL",
	description: "anti spam messages",
			 ...
			 if(findThread.status){
					if (!userSpamBlocker.canSend(userId,findThread)) {
					 const name = (await Users.getData(userId)).name
						 api.sendMessage(`Spamming, kicked ${name}`, event.threadID);
...
try{
	...
	if(args[0] === "on") {
		if(!findThread) return api.sendMessage(`No data yet`, event.threadID, event.messageID)
	...
	if (isNaN(count) || isNaN(time)) return api.sendMessage("Wrong format", event.threadID)
	if(!findThread) {
		data.push({
		...
		return api.sendMessage(`✅ Done. Use ${global.config.PREFIX + "" + this.config.name} on to enable`, event.threadID)
	}else {
		return api.sendMessage(`React '❤️' to confirm change`, event.threadID, (err,info) => {
