module.exports.config = {
  ...
  description: "Create an image of all group members",
  commandCategory: "User",
  usages: "family <size> [#color] or family <size>\nEnter suitable avatar size and text color (default black) using syntax:\n$family <size> <color> <title>\nWhere:\n•size: avatar size\n•color: hex color code\n•title: image title, default is group name\nExample: $family 200 #ffffff One Family\nIf size = 0, auto-adjust; if no title, use group name",
  cooldowns: 5,
  dependencies: {
    "fs-extra": "", 
    "axios":"", 
...
var live = [], admin = [], i = 0;
if(args[0] == 'help' || args[0] == '0' || args[0] == '-h') return api.sendMessage('Usage: '+ this.config.name + ' [avatar size]' + ' [color]' + ' [group title] || leave blank for auto', threadID, messageID)
/*============DOWNLOAD FONTS=============*/
...
size = size-l*2;
/*================CREATE PATH AVATAR===============*/
api.sendMessage(`🍗 Estimated avatars: ${participantIDs.length}\n🍠 Background size: ${bgX} x ${bgY}\n🥑 Avatar size: ${size}\n🥪 Color: ${color}\n🔄 Drawing in progress`,threadID, messageID);
...
/*=================DRAW AVATAR MEMBERS==============*/
  ...
  console.log(`Filtered ${participantIDs.length-i} Facebook users`)
  const cutImage = await jimp.read(imgCanvas.toBuffer());
  cutImage.crop(0, 0, xcrop, ycrop+l-30).writeAsync(pathAVT);
  await delay(300);
  /*====================SEND IMAGE==================*/ 
  return api.sendMessage({body: `🍗 Members: ${i}\n🥪 Background size: ${bgX} x ${bgY}\n🍠 Filtered ${participantIDs.length-i} Facebook users`, attachment: fs.createReadStream(pathAVT)}, threadID, (error, info) =>{
    if (error) return api.sendMessage(`An error occurred ${error}`, threadID, () => fs.unlinkSync(pathAVT), messageID)
