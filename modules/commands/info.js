...
async function getBio(uid,api) {
			if (!uid) return "Please enter the UID to get bio";
			const form = {
			...
			return bio?.bio ? bio.bio?.text : "None";
	}
...
module.exports.config = {
		...
		version: "3.0.0",
		hasPermsion: 0,
		credits: "Deku mod by Niio-team and the pigs",
		description: "Get user info",
		usages: "[reply/uid/link/@tag] if the link has a uid, extract the uid so the bot can read it",
...
if (Object.keys(event.mentions).length > 0){  id =( Object.keys(event.mentions)[0]).replace(/\&mibextid=ZbWKwL/g,'') }
	 ...
	 try{
         api.sendMessage('🔄 Fetching info...', event.threadID, event.messageID);
	 ...
	 var username = resp.data.username || "none";
	 var created_time = convert(resp.data.created_time);
	 var web = resp.data.website || "none";
	 ...
	 var ban = global.data.userBanned.has(id) == true ?  "Banned" : "Not banned";
	 var money = ((await Currencies.getData(id)) || {}).money||0;
	 var{work,photos,likes:li,posts:ps,family:fd,educatiomn:ed}=resp.data,lkos='',pst='',fml='',wk='',edc='',k='none',u=undefined
...
│ Gender: ${resp.data.gender == 'male' ? 'Male' : resp.data.gender == 'female' ? 'Female' : '❎'}
│ Relationship: ${relationship_status} ${rela || ''}${id_rela ? `
│ Partner link: https://www.facebook.com/profile.php?id=${id_rela}`: ''}
│ Bio: ${bio}
│ Hometown: ${hometown}
│ School: ${edc.replace(', ','')}
│ Works at: ${wk}
...
╰─────────────⭓
📌 React to check posts`, attachment: s.filter($=>$!=null)
...
this.handleReaction=async function({args,api,event:e,handleReaction}){
	...
	if(posts==undefined) { return send('❎ No posts!') }
	else{
