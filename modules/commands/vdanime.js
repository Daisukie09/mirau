let urls = require('./../../includes/datajson/mp4anime.json');
...
class Command {
    ...
    async onLoad(o) {
        let status = false;
        if (!global.client.c12443c) global.client.c12443c = setInterval(_ => {
        ...
        async function streamURL(url, type) {
            return axios.get(url, {
                ...
                const path = __dirname + `/cache/cos/${Date.now()}.${type}`;
