...
module.exports.run = async function ({ api, event }) {
    if (!event.messageReply || !event.messageReply.attachments) {
        return api.sendMessage("⚠️ Invalid image, please reply to an image", event.threadID, event.messageID);
    }

    const attachments = event.messageReply.attachments;
    if (attachments.length === 0) {
        return api.sendMessage("⚠️ No image was replied to", event.threadID, event.messageID);
    }

    const imageAttachment = attachments[0];
    if (!imageAttachment.url) {
        return api.sendMessage("⚠️ Image URL not found", event.threadID, event.messageID);
    }
    ...
    try {
        ...
        if (!imageUrl) {
            return api.sendMessage("⚠️ Cannot sharpen image", event.threadID, event.messageID);
        }
