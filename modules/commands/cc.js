...
exports.config = {
    ...
    description: 'Capcut platform info',
...
exports.run = async function ({ api, event, args }) {
    ...
    switch (command) {
        case 'info':
            const url = argument;
            if (!url) {
                return api.sendMessage("Please provide the creator's profile link!", tid, mid);
            }
            if (!url.startsWith("https://mobile.capcutshare.com/")) {
                return api.sendMessage("❎ Invalid URL. Please enter a URL in the format https://mobile.capcutshare.com", tid, mid);
            }
            try {
                ...
                const tiktokInfo = user.is_display_tiktok_profile ? `On\n│ Link: ${user.tiktok_user_info.deeplink}` : "Off";
                const certificationDesc = user.creator_info.certification_desc ? `${user.creator_info.certification_desc}` : "None";
...
│ Banned: ${user.ban ? "Yes" : "No"}
│ Total templates: ${userStatistics.template_count}
│ Role: ${user.role}
${vipInfo ? `│ VIP package: ${convertTime(vipInfo.start_time)} - ${convertTime(vipInfo.end_time)}` : "Not subscribed"}
├─────────────⭔
│ Creator info
...
├─────────────⭔
│ Showing TikTok profile: ${tiktokInfo}
╰─────────────⭓`, attachment: await streamURL(user.avatar_url, 'jpg')
                }, tid, mid);
            } catch (error) {
                console.error(error);
                api.sendMessage("❎ User data not found", tid, mid);
            }
            break;
        case 'search':
            try {
                ...
                if (!data || !data.video_templates || data.video_templates.length === 0) {
                    api.sendMessage("No results found.", tid, mid);
                    return;
                }
                ...
                api.sendMessage("An error occurred, please try again later.", tid, mid);
            }
            break;
        // Other cases
        // ...
        ...
        default:
            api.sendMessage("Usage guide", tid, mid);
            break;
    }
};
...
module.exports.handleReply = async function ({ event, api, handleReply, args }) {
    ...
    switch (handleReply.type) {
        case 'search':
            ...
            if (isNaN(choose)) {
                return api.sendMessage('⚠️ Please enter a number', tid, mid);
            }
            if (choose > 6 || choose < 1) {
                return api.sendMessage('❎ Selection is not in the list', tid, mid);
            }
            try {
                ...
                api.sendMessage({
                    body: `[ Capcut Video Info ]\n──────────────────\n|› Title: ${chosenVideo.title}\n|› Author: ${chosenVideo.author.name} (${chosenVideo.author.unique_id})\n|› Duration: ${formatTime(chosenVideo.duration)} seconds\n|› Images needed: ${chosenVideo.fragment_count}\n|› Uses: ${chosenVideo.usage_amount}\n|› Views: ${chosenVideo.play_amount}\n|› Likes: ${chosenVideo.like_count}\n|› Comments: ${chosenVideo.interaction.comment_count}\n|› Favorites: ${chosenVideo.favorite_count}\n|› Upload date: ${moment.unix(chosenVideo.create_time).tz('Asia/Ho_Chi_Minh').format('HH:mm:ss - DD/MM/YYYY')}\n|› Template link: https://www.capcut.com/template-detail/${chosenVideo.id}\n──────────────────\n|› Time: ${moment.tz("Asia/Ho_Chi_Minh").format("DD/MM/YYYY || HH:mm:ss")}`,
                    attachment: videoData
                }, tid, mid);
            } catch (error) {
                console.error("Error:", error.message);
                api.sendMessage("An error occurred while downloading the video.", tid, mid);
            }
            break;
        default:
            break;
...
async function search(keyword) {
    if (!keyword) throw new Error('Missing data to start program');
    const options = {
        method: 'POST',
