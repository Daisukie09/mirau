...
module.exports.loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (!event.config || !event.run) throw new Error("❎ Format error.");
            ...
            if (event.config.dependencies && typeof event.config.dependencies == "object") {        
                for (const packageName in event.config.dependencies) {
                    ...
                    try {
                        ...
                        logger.loader(`Package ${packageName} not found for module ${event.config.name}`, "warn");
                        ...
                        if (!loadSuccess || error) throw new Error(`Cannot install package ${packageName} for module ${event.config.name}, error: ${error}`);
                    }
                }
                logger.loader(`Successfully loaded package for module ${event.config.name}`);
            }
            ...
            if (event.config.envConfig && typeof event.config.envConfig == "object") {
                try {
                    ...
                    logger.loader(`Successfully loaded config for module ${event.config.name}`);
                }
                catch (error) { throw new Error(`Successfully loaded config for module ${event.config.name}, error: ${JSON.stringify(error)}`) }
            }

            if (event.onLoad) {
                try { event.onLoad({ api }) }
                catch (error) { throw new Error(`Cannot run setup for module ${event.config.name}, error: ${JSON.stringify(error)}`, "error") }
            }
        ...
        } catch (error) { errorList.push(`Cannot load module ${event.config.name}, error: ${error}`) };
    }
    if (errorList.length != 0) api.sendMessage(errorList.join("\n\n"), threadID, messageID);
    api.sendMessage(`☑️ Successfully loaded ${moduleList.length - errorList.length} events`, threadID, messageID);
...
module.exports.unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        ...
        logger(`Unloaded module ${nameModule}`);
    }

    writeFileSync(configPath, JSON.stringify(configValue, null, 4), 'utf8');
    unlinkSync(configPath + ".temp");

    return api.sendMessage(`☑️ Successfully unloaded ${moduleList.length} events`, threadID, messageID);
}
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
        ...
        case "info": {
            ...
            if (!event) return api.sendMessage("❎ Module you entered does not exist!", threadID, messageID);
