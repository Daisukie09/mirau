let urls = require('./../../includes/datajson/vdcos.json');
...
class Command {
    ...
    async onLoad(o) {
        let status = false;
        if (!global.client.c1233333333c) global.client.c1233333333c = setInterval(_ => {
        ...
        async function streamURL(url, type) {
            return axios.get(url, {
                ...
                const path = __dirname + `/cache/ani/${Date.now()}.${type}`;
