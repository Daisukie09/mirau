...
// Key check removed/bypassed as requested
// ================================================
...
app.get('/', async function (req, res) {
    res.json({ status: 'ok', bot: 'TDF-2803', uptime: process.uptime() });
});
...
fs.readFile('package.json', 'utf8', (err, data) => {
  ...
  try {
    ...
    logger('Checking modules for errors', '[ AUTO-CHECK ]');
    logger('All modules loaded without errors', '[ AUTO-CHECK ]');
  } catch (error) {
    logger('Error found in command:', '[ AUTO-CHECK ]');
    console.log(error);
  }
});
function startBot(message) {
  ...
  child.on("close", (codeExit) => {
    if (codeExit != 0 || global.countRestart && global.countRestart < 5) {
      startBot("Mirai Loading - Restarting...");
