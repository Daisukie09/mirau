        ...
        try {
            ...
            switch (logMessageType) {
              ...
              dataThread.adminIDs = dataThread.adminIDs.filter(item => item.id !== targetID);
                    ...
                    logger(`Update Admin List For Group: ${threadID}`, '[ UPDATE DATABASE ]');
        await setData(threadID, { threadInfo: dataThread });
         break;
     }
           case "log:thread-name": {
                    const newThreadName = logMessageData.name;
                    logger(`Update Group Name For Group: ${threadID}`, '[ UPDATE DATABASE ]');
     ...
     /* case "log:subscribe": {
       ...
       await delData(threadID);
                ...
                const participantIndex = dataThread.participantIDs.indexOf(leftParticipantFbId);
                        ...
                        logger(`Deleting User Data: ${leftParticipantFbId} | In Group: ${threadID}`, '[ UPDATE DATABASE ]');
                        await setData(threadID, { threadInfo: dataThread });
                    }
                    break;
                }
