module.exports = function ({ api, models }) {
  ...
  /////////////////////////////////////////////////////////////////////////////
  //========= AUTO-CLEAR DATA - Auto clear cache every 6h or when > 1GB =========//
  /////////////////////////////////////////////////////////////////////////////
  ...
  // Auto-clear interval - runs every 30 minutes to check
  setInterval(function () {
    try {
      // Check auto-clear config
      ...
      // Clear if 6h passed or cache > 1GB
        ...
        const reason = cacheSize >= ONE_GB ? 'size > 1GB' : '6 hours passed';
        logger(`Auto-Clear: Deleted ${count} files (${(clearedSize / 1048576).toFixed(2)} MB) - Reason: ${reason}`, '[ AUTO-CLEAR ]');
      }
    } catch (e) {
      // Silent fail
    }
  ...
  logger(" TDF-2803 BOT | zL: 0878139888 | Accepting bot customization requests", "[ TDF-2803 ]");
  logger("┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓", "[ BOT INFO ]");
  ...
  //FUNCTION WORKS AS ITS NAME SUGGESTS, CRE: DUNGUWU
  const monthToMSObj = {
    1: 31 * 24 * 60 * 60 * 1000,
    2: 28 * 24 * 60 * 60 * 1000,
    3: 31 * 24 * 60 * 60 * 1000,
  ...
  const checkTime = (time) => new Promise((resolve) => {
    ...
    if (time[4] > 59 || time[3] < 0) resolve("[🦑]➜ Invalid minute");
    if (time[5] > 59 || time[3] < 0) resolve("[🦑]➜ Invalid second");
    yr = time[2] - 1970;
    ...
    if (logMessageType === "log:thread-name") {
      ...
      if (findAnti) {
        if (findAd || botID.includes(author)) {
          ...
          api.sendMessage(`⚠️ Anti-group-name-change triggered\n⏰ Time: ${moment().tz("Asia/Ho_Chi_Minh").format("HH:mm:ss || DD/MM/YYYY")}`, threadID);
    ...
    if (logMessageType === "log:user-nickname") {
      ...
      if (findAnti) {
        if (findAd || botID.includes(author)) {
          ...
          api.sendMessage(`⚠️ Anti-nickname-change triggered\n⏰ Time: ${moment().tz("Asia/Ho_Chi_Minh").format("HH:mm:ss || DD/MM/YYYY")}`, threadID);
      ...
      case "event":
        ...
        if (event.type != "change_thread_image" && global.config.notiGroup) {
          ...
          if (event.author == api.getCurrentUserID()) {
            hhh = msg.replace('You', global.config.BOTNAME)
          }
...
////////////////
/// TDF-2803 Bot by Trịnh Đình Phát
