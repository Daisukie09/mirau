...
express.set('DFP', (parseInt(process.env.PORT || process.env.port) + 1 || 8081));
...
function buildAPI(globalOptions, html, jar, bypass_region) {
    //new feat
    const fb_dtsg = utils.getFroms(html, '["DTSGInitData",[],{"token":"', '","')[0];     //grab grab grab grab
    ...
    if (maybeUser.length === 0 && maybeTiktik.length === 0) {
        ...
        // compile list of Kanzu's never-die regions =))
        /**
