...
if (global.config.autoCleanCache.Enable) {
  ...
  fs.unlink(filePath, (err) => {
          if (err) {
logger(`Error cleaning files jpg, mp4, gif, ttf, mp3`, "[ AUTO - CLEAN ]", err);
           } else {
         }
      });
    }
});
logger(`Cleaned jpg, mp4, gif, ttf, mp3 files`, "[ AUTO - CLEAN ]");
  });
} else {
logger(`Auto Clean Cache is Disabled`, "[ AUTO - CLEAN ]");
}
////////////////////////////////////////////////
//========= Login to account, start listening for events & auto-get Appstate from config =========//
////////////////////////////////////////////////
...
function onBot({ models }) {
  ...
  login(loginData, async (loginError, loginApiData) => {    
    if (loginError) {
      logger('Cannot login with appState, trying Facebook password login!','[ LOGIN-ERROR ]')
      var loginauto = await loginAppstate()
      loginauto
      await new Promise((reset) => setTimeout(reset, 7000))
      logger('Restarting!', '[ RESTART ]')
      process.exit(1)
    }
